...
import edu.mirea.onebeattrue.samsunghack.presentation.auth.otp.OtpStore.State
import edu.mirea.onebeattrue.samsunghack.presentation.auth.otp.OtpStore.Intent
import edu.mirea.onebeattrue.samsunghack.presentation.auth.otp.OtpStore.Label
        ...
        override fun executeIntent(intent: Intent, getState: () -> State) {
            when (intent) {
                ...
                Intent.ResendOtp -> {
                    dispatch(Msg.Loading)
                    scope.launch {
                        authRepository.resendVerificationCode().collect { result ->
                            when (result) {
                                is AuthState.Failure -> {
                                    dispatch(Msg.Failure(result.exception.message.toString()))
                                }
